...
class TodoDetailView(View):
    ...
    def post(self, request, task_id):
        ...
        if 'save' in request.POST:
            ...
            task.delete()
        elif 'complete' in request.POST: task.update(completed=TRUE)
        # "redirect" to the todo homepage
        return redirect('todo_list')

class NoteListView(View):
    def get(self, request):
        '''GET the todo list homepage, list all tasks in reverse order created'''
    ...
    def post(self, request):
        'POST the data per user input, creating a new task in todo list'
        form=NoteForm(request.POST)
        if form.is_valid():
            note_text = form.cleaned_data['text']
            Note.objects.create(text=note_text)

        #todo homepage reroute
        return redirect('todo_list')
